...
def GetDataset(name, base_path):
    ...
    base_path : string, e.g. "path/to/datasets/directory/" where to save the dataset
    ...
    if name == 'mnist':
        ...
        # craete containers fo the features and labels
        X = np.zeros((28 * 28, len(train_loader)), dtype=np.float32)
        y = np.zeros(len(train_loader), dtype=np.int)

        # create matrix (p X n )  of features and vector of labels
        for i, (data, target) in enumerate(train_loader):
            X[:, i] = data.view(-1)
            y[i] = target

        # subtract minimum why?
        y = y - min(y)

        # reduce dimension to 50
        pca = PCA(n_components=50)
        X = pca.fit_transform(X.T)

        # normalize to zero mean and variance 1

...
# class for the first model from the experiments
class Model_Ex1:
    # function to initialize model
    def __init__(self, K, p, magnitude=1):
        self.K = K                  # number of classes
        self.p = p                  # dimension of data
        self.magnitude = magnitude  # std of randomized parameters

        # Generate model parameters as a randomized linear weights matrix
        self.beta_Z = self.magnitude * np.random.randn(self.p, self.K)

    # function to create dataset with n points of dimension p
    def sample_X(self, n):
        # the dataset consist of independent Gaussian vectors of dimension p (with independent coordinates)
    ...
    # function to compute the model conditional probability
    def compute_prob(self, X):
        # apply the model on the points in the dataset
        f = np.matmul(X, self.beta_Z)

        # apply softmax function to get probabilities vectors
        prob = np.exp(f)
        prob_y = prob / np.expand_dims(np.sum(prob, 1), 1)
        return prob_y

    # function to sample the labels from the model conditional probability
    def sample_Y(self, X):
        # get model conditional probailities
...
# class for the second model from the experiments
class Model_Ex2:
    # function to initialize model
    def __init__(self, K, p, magnitude=1):
        self.K = K                  # number of classes
